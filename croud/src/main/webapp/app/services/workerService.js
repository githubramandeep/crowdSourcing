mainApp.service('workerService', function($http, $q, $rootScope, $cookies) {
	...
	this.getWorkerDetailsByUsername = function() {
		...
		$http.get(
				'http://localhost:8080/croud/rest/worker/username/'
				+ user, config).success(
	...
	this.getPendingRequests = function() {
		...
		$http.get(
				'http://localhost:8080/croud/rest/workerTaskDetails/pendingReq/workerId/'
	...
	this.assignTaskReq = function(data) {
		...
		var workerId= $cookies.get('workerId')
		$http.post(
				'http://localhost:8080/croud/rest/workerTaskDetails/assignTask/workerId/'
	...
	this.getCompletedTasks = function() {
		...
		var workerId= $cookies.get('workerId')
		$http.get(
				'http://localhost:8080/croud/rest/workerTaskDetails/id/'
				+ workerId, config).success(		
				function(result, status, headers, config) {
