...
mainApp.controller("adminDashboardCtrl",
		function($scope, $resource, $http, categoryService) {
			...
			$scope.sendPost = function() {
				...
				$http.post('http://localhost:8080/croud/rest/category/create',
						data, config).success(
			...
			$scope.deleteCategory = function(categorypk) {
				...
				$http['delete']('http://localhost:8080/croud/rest/category/delete/id/'+
			...
			function getCategories() {
				...
				$http.get('http://localhost:8080/croud/rest/category/categoryLst'
					, config).success(
						function(data, status, headers, config) {
